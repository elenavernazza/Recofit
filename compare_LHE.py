...
color1_RECO = ROOT.TColor.GetFreeColorIndex()
col1 = ROOT.TColor(color1_RECO, 1./255., 119./255., 193./255.)
color2_RECO = ROOT.TColor.GetFreeColorIndex()
col2 = ROOT.TColor(color2_RECO, 113./255., 207./255., 235./255.)

color1_LHE = ROOT.TColor.GetFreeColorIndex()
col3 = ROOT.TColor(color1_LHE, 241./255., 80./255., 98./255.)
color2_LHE = ROOT.TColor.GetFreeColorIndex()
col4 = ROOT.TColor(color2_LHE, 245./255., 155./255., 146./255.)
...
if __name__ == "__main__":
    ...
    parser.add_argument('--graphLimits',     dest='graphLimits',     help='comma separated list of final graph y axis limits, default is 14,18', required = False, default="10,14")
    ...
    for model in best.keys():
        ...
        g1.SetMinimum(-10)
        g1.SetMaximum(15)
        g1.SetFillColor(color1_RECO)
        g1.SetLineColor(color1_RECO)
        g1.SetLineWidth(0)
        ...
        g11.SetMinimum(-10)
        g11.SetMaximum(15)
        g11.SetFillColor(color1_RECO)
        g11.SetLineColor(color1_RECO)
        g11.SetLineWidth(0)

        g2.SetFillColor(color2_RECO)
        g2.SetLineWidth(0)
        ...
        g21.SetMinimum(-10)
        g21.SetMaximum(15)
        g21.SetFillColor(color2_RECO)
        g21.SetLineColor(color2_RECO)
        g21.SetLineWidth(0)

   
        g3.SetMinimum(-10)
        g3.SetMaximum(15)
        g3.SetFillColor(color1_LHE)
        g3.SetLineColor(color1_LHE)
        g3.SetLineWidth(0)
        ...
        g31.SetMinimum(-10)
        g31.SetMaximum(15)
        g31.SetFillColor(color1_LHE)
        g31.SetLineColor(color1_LHE)
        g31.SetLineWidth(0)

        g4.SetFillColor(color2_LHE)
        g4.SetLineWidth(0)
        g4.SetMarkerStyle(24)
        g4.SetMarkerColor(ROOT.kRed+1)
        g4.SetMarkerSize(1)
        g4.GetYaxis().SetRangeUser(-15,15)

        g41.SetMinimum(-10)
        g41.SetMaximum(15)
        g41.SetFillColor(color2_LHE)
        g41.SetLineColor(color2_LHE)
        g41.SetLineWidth(0)

        # leg.AddEntry(g1, "#pm 1#sigma with rateParam", "F")
        # leg.AddEntry(g2, "#pm 2#sigma with rateParam", "F")
        # leg.AddEntry(g3, "#pm 1#sigma no rateParam", "F")
        # leg.AddEntry(g4, "#pm 2#sigma no rateParam", "F")

        leg.AddEntry(g1, "#pm 1#sigma reco level", "F")
        leg.AddEntry(g2, "#pm 2#sigma reco level", "F")
        leg.AddEntry(g3, "#pm 1#sigma LHE level", "F")
        leg.AddEntry(g4, "#pm 2#sigma LHE level", "F")

        leg.AddEntry(g1, "Best Fit reco", "P")
        leg.AddEntry(g3, "Best Fit LHE", "P")
        leg.SetBorderSize(2)
...
#        cpm.lumi = str(args.lumi + " fb^{-1}")
        ...
        for op, one_e, one_q in zip(ops, one_s_e, one_s_q):
            if type(one_e[0])==float and type(one_e[1])==float and type(one_q[0])==float and type(one_q[1])==float :
                print("{} & {}".format(op, float((one_e[1]-one_e[0])/(one_q[1]-one_q[0]))))
        ...
        if args.drawText:
            for x_e,x_q,y_e,y_q,var_e,var_q in zip(xs, xs_qcd, two_s_e, two_s_q, vars_e, vars_q):
                if type(y_e[1]) == float :
                    ...
                    y_up = max(y_e[0][1],y_e[1][1]) + 0.5
                if type(y_q[1]) == float :
                    y_do = y_q[0] - 0.5
                else :
                    y_do = min(y_q[0][0],y_q[1][0]) - 0.5
                ...
                if y_up < final_plot_y_max :
                    latex.DrawLatex(x_e+0.05 - 0.02*len(convertName(var_e)),y_up,"{}".format(convertName(var_e)))
                if y_do > final_plot_y_min :
                    latex.DrawLatex(x_q+0.1 - 0.02*len(convertName(var_q)),y_do,"{}".format(convertName(var_q)))

        c.Draw()
        if args.add: 
            c.Print(outputFolder_ewk + "/{}_LHE_{}.png".format(model,args.add))
            c.Print(outputFolder_ewk + "/{}_LHE_{}.pdf".format(model,args.add))
        else:
            c.Print(outputFolder_ewk + "/{}_LHE.png".format(model))
            c.Print(outputFolder_ewk + "/{}_LHE.pdf".format(model))

...
if __name__ == "__main__":
    ...
    parser.add_argument('--graphLimits',     dest='graphLimits',     help='comma separated list of final graph y axis limits, default is 14,18', required = False, default="14,18")
    ...
    for model in best.keys():
        ...
        g3.SetMinimum(-10)
        g3.SetMaximum(15)
        g3.SetFillColor(ROOT.kCyan)
        g3.SetLineColor(ROOT.kCyan)
        g3.SetLineWidth(0)
        g3.SetMarkerStyle(24)
        g3.SetMarkerColor(ROOT.kBlue+1)
        g3.SetMarkerSize(1)
        g3.GetYaxis().SetRangeUser(-10,10)

        g31.SetMinimum(-10)
        g31.SetMaximum(15)
        g31.SetFillColor(ROOT.kCyan)
        g31.SetLineColor(ROOT.kCyan)
        g31.SetLineWidth(0)

        g4.SetFillColor(ROOT.kAzure)
        g4.SetLineWidth(0)
        g4.SetMarkerStyle(24)
        g4.SetMarkerColor(ROOT.kBlue+1)
        g4.SetMarkerSize(1)
        g4.GetYaxis().SetRangeUser(-15,15)

        g41.SetMinimum(-10)
        g41.SetMaximum(15)
        g41.SetFillColor(ROOT.kAzure)
        g41.SetLineColor(ROOT.kAzure)
        g41.SetLineWidth(0)
        ...
        leg.AddEntry(g1, "#pm 1#sigma QCDscale correlated", "F")
        leg.AddEntry(g2, "#pm 2#sigma QCDscale correlated", "F")
        leg.AddEntry(g3, "#pm 1#sigma QCDscale uncorrelated", "F")
        leg.AddEntry(g4, "#pm 2#sigma QCDscale uncorrelated", "F")
...
#        cpm.lumi = str(args.lumi + " fb^{-1}")
        ...
        if args.drawText:
            for x_e,x_q,y_e,y_q,var_e,var_q in zip(xs, xs_qcd, two_s_e, two_s_q, vars_e, vars_q):
                if type(y_e[1]) == float :
                    ...
                    y_up = max(y_e[0][1],y_e[1][1]) + 0.3
                if type(y_q[1]) == float :
                    y_do = y_q[0] - 0.3
                else :
                    y_do = min(y_q[0][0],y_q[1][0]) - 0.3
        ...
        c.Draw()
        if args.add: 
            c.Print(outputFolder_qcd + "/{}_correlation_{}.png".format(model,args.add))
        else:
            c.Print(outputFolder_qcd + "/{}_correlation.png".format(model))
